        ...
        <Reveal>
          <div className="mb-12 border-b border-gold/10 pb-8">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="font-label text-[10px] uppercase tracking-[0.35em] text-gold/60">
                {recentProject.eyebrow}
              </span>
            </div>
            <h2 className="font-display text-4xl sm:text-5xl lg:text-6xl tracking-wide text-foreground">
              {recentProject.title} &ndash;{" "}
              <em className="not-italic font-heading italic gold-gradient-text">{recentProject.titleItalic}</em>
            </h2>
          ...
          <div className="lg:col-span-6 flex flex-col justify-between h-full space-y-6 sm:space-y-8">
            ...
            <Reveal delay={2} className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 border-t border-gold/20 pt-6 sm:pt-8">
              {recentProject.highlights.map((h, i) => (
                ...
                >
                  <h4 className="font-display text-base sm:text-lg text-gold font-semibold">{h.title}</h4>
            ...
            <Reveal className="h-full w-full relative group/visual carved-frame-hover">
              ...
              >
                ...
                <div className="relative h-full aspect-[4/5] sm:aspect-auto min-h-[400px] sm:min-h-[480px] overflow-hidden border border-gold/20 bg-basalt shadow-elevated">
                  ...
                  {/* Cinematic dark-to-transparent overlay at bottom for crystal-clear readability */}
                  <div className="absolute inset-0 bg-gradient-to-t from-basalt/95 via-basalt/40 to-transparent pointer-events-none" />

                  {/* Overlay text */}
                  <div className="absolute bottom-5 sm:bottom-6 left-5 sm:left-6 right-5 sm:right-6 text-white z-20 flex justify-between items-end gap-3">
                    <div>
                      <span className="font-label text-[9px] uppercase tracking-[0.3em] text-gold-soft/80">
                        Delivered Landmark
                      </span>
                      <h3 className="font-display text-xl sm:text-2xl mt-1 text-white tracking-wide font-semibold drop-shadow-sm">
                        {recentProject.title} {recentProject.titleItalic}
                      </h3>
                    </div>
                    {/* COMPLETED badge */}
                    <span className="font-label text-[9px] sm:text-[10px] uppercase tracking-[0.25em] text-gold-bright px-3 py-1 border border-gold/40 bg-basalt/80 backdrop-blur-sm shadow-md font-bold shrink-0">
                      COMPLETED
                    </span>
                  </div>
                </div>
